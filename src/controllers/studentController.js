...
const update = async function (req, res) {
  try {
    ...
    if (name == check.name) return res.send("This name already exists.No need to update")
    updateData["name"] = name;


    if (marks == check.marks) return res.send("Marks are same.No need to update")
    updateData["marks"] = marks;

    let update = await studentModel.findOneAndUpdate({ _id: studentId }, updateData, { new: true, upsert: true });
...
const deleteStudent = async function (req, res) {
  try {
    ...
    let student = await studentModel.findOne({_id:studentId, isDeleted:true});
    if (!student) {
      return res.send({
        status: false,
...
module.exports = { student, update, viewStudent ,deleteStudent };
